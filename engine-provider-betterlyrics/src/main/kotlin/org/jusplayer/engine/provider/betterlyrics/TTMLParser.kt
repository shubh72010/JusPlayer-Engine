...
/**
 * DOM-based parser for Apple Music TTML lyrics (as served by the BetterLyrics
 * API). Line and word timings are resolved from the TTML timing context
 * ([frameRate], [frameRateMultiplier], [tickRate]), spanning clocks and
 * `h|m|s|ms|f|t` offsets. Child spans whose begin/end are relative to their
 * line are rebased to the line's own bounds.
 */
object TTMLParser {
        ...
        try {
            ...
            for (divIdx in 0 until divElements.length) {
                ...
                for (pIdx in 0 until pElements.length) {
                    ...
                    parsePElement(pElement, timingContext, transliterations, translations)?.let(lines::add)
                }
            }
    ...
    /** Converts parsed lines to the engine's [LyricsLine] model, in milliseconds. */
    fun toEngineLines(lines: List<ParsedLine>): List<LyricsLine> {
        return lines.map { line ->
            LyricsLine(
                text = line.text,
                start = toMillis(line.startTime),
                end = toMillis(line.endTime),
                words = line.words.map { word ->
                    LyricsWord(
                        text = word.text,
                        start = toMillis(word.startTime),
                        end = toMillis(word.endTime),
                    )
                },
            )
        }
    ...
    private fun toMillis(seconds: Double): Long = Math.round(seconds * 1000.0)
}
